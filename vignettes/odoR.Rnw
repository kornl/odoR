...
library(odoR)
file <- system.file("extdata", "Sterillium-100.csv", package = "odoR")
dat <- read.odor(file)
plot.odor(dat)
@
...
for (file in dir(system.file("extdata", package = "odoR"))) {
  ...
  dat$residualize()
  dat$spider_plot()
  filename_information <- strsplit(file, split=c("[-.]"))
  dataset$add_measurement(dat, filename_information[1], filename_information[2])
}
